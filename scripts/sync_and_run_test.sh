...
GDRIVE_QUEUE="gdrive:AI-Lab_Yue_Mao/rclone/queue/"
...
# Move files from subdirectories to queue root (but NEVER touch images/)
for subdir in "$QUEUE_DIR"/*/ ; do
    if [ -d "$subdir" ]; then
        if [ "$(basename "$subdir")" = "images" ]; then continue; fi
...
cd "$PROJECT_DIR"
job_id=$(sbatch scripts/run_pipeline_test.slurm | grep -oP '\d+')
